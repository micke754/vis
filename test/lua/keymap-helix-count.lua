...
describe("helix counts", function()
	...
	it("W deletes current WORD and trailing space", function()
		reset("foo bar baz\n")
		vis:feedkeys("Wd")
		assert.are.same("bar baz\n", content())
	end)

	it("E deletes current WORD without trailing space", function()
		reset("foo bar baz\n")
		vis:feedkeys("Ed")
		assert.are.same(" bar baz\n", content())
	end)

	it("B deletes previous WORD", function()
		reset("foo bar baz\n", 8)
		vis:feedkeys("Bd")
		assert.are.same("foo baz\n", content())
	end)
end)
