...
- [ ] Add jumplist picker if current state is accessible cleanly. Current-window jump only; no split/vsplit for jumplist items.
...
- Next implementation order: selected-line preview centering, jumplist picker, changed-file picker if clean, then syntax-highlighted preview attempt. If blocked, record status here and move on.
